// ShaderBackground — a zero-dependency WebGL canvas ("Mesh drift", generated by the 21st.dev
// Shader Builder) that fills its parent. Drop it behind content with:
//   <div className="relative"><ShaderBackground className="absolute inset-0" />…</div>
//
// The rendering engine (shaders, rAF loop, ResizeObserver, IntersectionObserver, visibility
// handling, DPR-aware sizing, context cleanup) is preserved verbatim from the supplied 21st.dev
// component. Only two things are DOCURA-specific: the UNIFORMS palette/motion (a dark Deep-Navy
// field with restrained lime energy, slow drift, cursor interaction OFF), and a reduced-motion
// guard that freezes the field to a single static frame while keeping the dark background.
...
// DOCURA configuration — a dark Deep-Navy field with restrained lime energy.
// Palette: three navy/charcoal tones dominate; a single dim olive-lime carries the only green
// energy. Motion is slow (timeScale + gentle drift), density is low (larger scale, low
// intensity), grain is minimal, a vignette quiets the edges, and cursor interaction is OFF.
const UNIFORMS = {
  colors: [
    [0.031, 0.047, 0.078], // #080C14 near-black navy (base / dominant)
    [0.055, 0.086, 0.141], // #0E1624 deep navy
    [0.094, 0.145, 0.208], // #182535 charcoal-navy
    [0.44, 0.6, 0.28], // #709947 dim, restrained DOCURA lime — the only green energy
    [0.44, 0.6, 0.28],
    [0.44, 0.6, 0.28],
    [0.44, 0.6, 0.28],
    [0.44, 0.6, 0.28],
  ...
  colorCount: 4,
  scale: 1.3, // larger, softer shapes → lower visual density
  intensity: 0.18, // calmer, tighter colour motion
  paramA: 0.5,
  warp: 0.0,
  detail: 2.4,
  contrast: 1.06,
  brightness: -0.02, // keep it dark overall
  saturation: 0.9, // slightly muted
  hue: 0.0,
  vignette: 0.28, // darken the edges so content frames cleanly
  blur: 0.0,
  grain: 0.035, // minimal grain
  seed: 1453.0,
  rotate: 0.0,
  offsetX: 0.0,
  offsetY: 0.0,
  drift: 0.02, // very gentle global wander
  cursorEnabled: false, // no cursor interaction — comfortable for reading
  cursorEffect: 2.0,
  cursorStrength: 0.65,
  cursorRadius: 0.46,
  oklab: 0.0,
  timeScale: 0.09, // slow, understated motion
}
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Respect reduced-motion: freeze the field to a single static frame (the dark background is
    // retained, only the animation stops). Evaluated once on mount.
    const prefersReducedMotion =
      typeof window.matchMedia === "function" &&
      window.matchMedia("(prefers-reduced-motion: reduce)").matches
    const timeScale = prefersReducedMotion ? 0 : UNIFORMS.timeScale
